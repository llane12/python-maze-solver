...
class Cell:
    ...
    def draw(self, window, line_color="black"):
        if self.has_left_wall:
            window.draw_line(Line(Point(self.p1.x, self.p1.y), Point(self.p1.x, self.p2.y)), line_color)
        if self.has_right_wall:
            window.draw_line(Line(Point(self.p2.x, self.p1.y), Point(self.p2.x, self.p2.y)), line_color)
        if self.has_top_wall:
            window.draw_line(Line(Point(self.p1.x, self.p1.y), Point(self.p2.x, self.p1.y)), line_color)
        if self.has_bottom_wall:
            window.draw_line(Line(Point(self.p1.x, self.p2.y), Point(self.p2.x, self.p2.y)), line_color)
